...
def transform_time(send_time: str = None):
    ...
    if send_time is None:
        ...
        try:
            ...
            try:
                # 尝试解析只有日期的格式
                send_time = datetime.strptime(send_time, '%Y-%m-%d')
                # 如果成功解析为日期格式，随机生成时分秒
                hour = random.randint(0, 23)
                minute = random.randint(0, 59)
                second = random.randint(0, 59)
                send_time = send_time.replace(hour=hour, minute=minute, second=second)
            except ValueError:
                # 如果都失败，抛出更明确的错误信息
                raise ValueError(f"不支持的日期格式: {send_time}。支持的格式: 'YYYY-MM-DD' 或 'YYYY-MM-DD HH:MM:SS'")
...
if __name__ == "__main__":
    ...
    # print(response)
    print(transform_time("2025-06-27"))