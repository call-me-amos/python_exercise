...
class Logger(object):
    ...
    logname = os.path.join(logpath, sys.argv[0].split('/')[-1].split('.')[0] + '_' + datetime.now().strftime("%Y%m") + '.log')

    # 初始化logger
    log = logging.getLogger()
    # 日志格式，可以根据需要设置
    fmt = logging.Formatter('[%(asctime)s][%(filename)s][line:%(lineno)d][%(funcName)s][%(levelname)s] %(message)s',
                            '%Y-%m-%d %H:%M:%S')
    ...
    # def info(self, msg):
    #     # 替换回车换行符
    #     # self.log.info(msg.replace('\r', ' ').replace('\n', ' ').replace('\t', ' '))
    #     self.log.info(msg)
    #     return
    #
    # @classmethod
    ...
    #     # 替换回车换行符
    #     # self.log.warning(msg.replace('\r', ' ').replace('\n', ' ').replace('\t', ' '))
    ...
    # def error(self, msg):
    #     # 替换回车换行符
    #     # self.log.error(msg.replace('\r', ' ').replace('\n', ' ').replace('\t', ' '))
    #     self.log.error(msg)
    #     return
