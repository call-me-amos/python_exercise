...
def process_all_rows(max_row, api_key, fastgpt_api_url, data_list=None):
    ...
    for index, item in enumerate(data_list):
        try:
            ...
            print(f"当前fastgpt处理行 知识-专业性建议问题：{index}")
            ...
            # 专业性建议
            pro_suggestion = ''
            if '专业度承接-时间压缩' in responseData_map and responseData_map['专业度承接-时间压缩'] is not None and len(responseData_map['专业度承接-时间压缩']) > 0:
            ...
            print(f"index={index}，数据解析异常 e={ep}")
            continue
    return results
